...
- (id)handlerForIntent:(INIntent *)intent {
    ...
    if ([intent isKindOfClass:[INSendMessageIntent class]]) {
        ...
        NSLog(@"use IntentHandle  to handler intent");
    }else{
        
        NSLog(@"other object to handle intent");
    }
...
- (void)confirmSendMessage:(INSendMessageIntent *)intent completion:(void (^)(INSendMessageIntentResponse *response))completion {
    ...
    NSUserActivity *userActivity = [[NSUserActivity alloc] initWithActivityType:@"confirm"];
...
//    NSUserDefaults *userDefault = [[NSUserDefaults alloc]initWithSuiteName:@"group.siiiriEx"];
//    NSString *string = [userDefault objectForKey:@"name"];
//    NSLog(@"%@",string);
//    
    
    //1、excute in extension directly
    //2、mainAPP响应发送结果，how?
        //not avalibale in extension
//    UIViewController  *viewC = (UIViewController *)[UIApplication sharedApplication].;
    

    NSUserActivity *userActivity = [[NSUserActivity alloc] initWithActivityType:@"handle"];
